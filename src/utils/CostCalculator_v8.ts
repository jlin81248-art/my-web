...
class CostCalculator {
  ...
  private generateRiskWarnings(after: OfferMetrics): RiskWarning[] {
    ...
    warnings.push(
      after.familyBurdenRate >= 25
        ? {
            level: '高',
            title: '家庭现金流风险',
            reason: '教育、赡养、医疗占比偏高，遇到额外支出时缓冲空间不足。',
            suggestion: '更适合优先谈稳定现金流和报销比例，而不是只看名义总包。',
          }
        : after.familyBurdenRate >= 15
          ? {
              level: '中',
              title: '家庭现金流风险',
              reason: '家庭支出占比不低，需要更关注结余和备用金。',
              suggestion: '这份 offer 可以成立，但前提是不要再叠加更高房租和更长通勤。',
            }
          : {
              level: '低',
              title: '家庭现金流风险',
    ...
    warnings.push(
      after.topPrioritySubjectiveScore < 3
        ? {
            level: '高',
            title: '预期落差风险',
            reason: '你最看重的 TOP3 维度主观体验分偏低，说明“看起来值”但未必“过起来值”。',
            suggestion: '重点检查你最在意的 3 个维度里，是否有一个维度正在明显掉分。',
          }
        : after.topPrioritySubjectiveScore < 3.6
          ? {
              level: '中',
              title: '预期落差风险',
