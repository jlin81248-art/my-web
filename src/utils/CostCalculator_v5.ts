...
class CostCalculator {
  ...
  calculateOfferMetrics(annualPackage: number, cityData: CityData, settings: Settings): OfferMetrics {
    ...
    const healthFriendlyScore = this.calculateHealthFriendlyScore(settings, {
      workHoursPerDay,
  ...
  getPreferenceWeights(order: PreferenceKey[]): PreferenceWeight[] {
    const distribution = [0.26, 0.20, 0.16, 0.12, 0.10, 0.08, 0.05, 0.03];
  ...
  calculateMonthlyCosts(cityData: CityData, settings: Settings): MonthlyCosts {
    ...
    const medical = Math.max(0, (settings.familyMedicalMonthly || 0) * (1 - reimbursementRate));
    ...
    const rows: PainPointRow[] = [
      {
        key: 'salary',
        label: '薪资与存钱',
        beforeValue: `${this.formatCurrency(before.monthlyDisposable)}/月可支配｜在岗 ${this.formatCurrency(before.workOnlyHourlyPay)}/h｜含通勤 ${this.formatCurrency(before.realHourlyPay)}/h`,
        afterValue: `${this.formatCurrency(after.monthlyDisposable)}/月可支配｜在岗 ${this.formatCurrency(after.workOnlyHourlyPay)}/h｜含通勤 ${this.formatCurrency(after.realHourlyPay)}/h`,
        deltaText: `${this.getSignedCurrency(after.monthlyDisposable - before.monthlyDisposable)} / 月可支配；在岗 ${this.getSignedCurrency(after.workOnlyHourlyPay - before.workOnlyHourlyPay)} / h；含通勤 ${this.getSignedCurrency(after.realHourlyPay - before.realHourlyPay)} / h`,
        impact: this.getImpact(after.monthlyDisposable - before.monthlyDisposable + (after.workOnlyHourlyPay - before.workOnlyHourlyPay) * 100),
        explanation: after.monthlyDisposable >= before.monthlyDisposable
          ? '跳槽后税后结余更高，而且按实际月工作天数均摊后，在岗时薪也更有优势。'
          : '虽然总包可能上涨，但涨薪没有有效转化成更高的月结余或更好的时薪。',
        priorityRank: rankMap.get('salary') || 99,
      },
      {
        key: 'entertainment',
        label: '娱乐与生活丰富度',
        beforeValue: `娱乐预算占比 ${before.entertainmentBudgetRatio}%｜环境 ${before.entertainmentSceneScore}/5｜自由时间 ${before.freeHoursPerDay}h`,
        afterValue: `娱乐预算占比 ${after.entertainmentBudgetRatio}%｜环境 ${after.entertainmentSceneScore}/5｜自由时间 ${after.freeHoursPerDay}h`,
        deltaText: `预算占比 ${this.getSignedPercent(after.entertainmentBudgetRatio - before.entertainmentBudgetRatio)}；环境 ${this.getSignedNumber(after.entertainmentSceneScore - before.entertainmentSceneScore)}；自由时间 ${this.getSignedNumber(after.freeHoursPerDay - before.freeHoursPerDay)}h`,
      ...
      },
      {
        key: 'time',
        label: '工时、通勤与自由时间',
        beforeValue: `月工作 ${before.workingDaysPerMonth} 天｜通勤 ${before.commuteMinutesPerDay} 分钟｜总占用 ${before.occupiedHoursPerDay}h/天`,
        afterValue: `月工作 ${after.workingDaysPerMonth} 天｜通勤 ${after.commuteMinutesPerDay} 分钟｜总占用 ${after.occupiedHoursPerDay}h/天`,
        deltaText: `月工作 ${this.getSignedNumber(after.workingDaysPerMonth - before.workingDaysPerMonth)} 天；通勤 ${this.getSignedNumber(after.commuteMinutesPerDay - before.commuteMinutesPerDay)} 分钟；自由时间 ${this.getSignedNumber(after.freeHoursPerDay - before.freeHoursPerDay)}h`,
        impact: this.getImpact((after.freeHoursPerDay - before.freeHoursPerDay) * 10 - (after.commuteMinutesPerDay - before.commuteMinutesPerDay) / 6 - (after.workingDaysPerMonth - before.workingDaysPerMonth) * 2),
        explanation: after.freeHoursPerDay >= before.freeHoursPerDay && after.commuteMinutesPerDay <= before.commuteMinutesPerDay
          ? '跳槽后月工作天数或通勤更友好，下班后的真实可支配时间更多。'
          : '跳槽后月工作天数、通勤或总占用时长更高，生活体感会被明显拉低。',
      ...
      },
      {
        key: 'health',
        label: '身心健康与工作强度',
        beforeValue: `健康友好度 ${before.healthFriendlyScore}/5｜工时 ${before.workHoursPerDay}h｜自由时间 ${before.freeHoursPerDay}h`,
        afterValue: `健康友好度 ${after.healthFriendlyScore}/5｜工时 ${after.workHoursPerDay}h｜自由时间 ${after.freeHoursPerDay}h`,
        deltaText: `健康友好度 ${this.getSignedNumber(after.healthFriendlyScore - before.healthFriendlyScore)}；工时 ${this.getSignedNumber(after.workHoursPerDay - before.workHoursPerDay)}h；自由时间 ${this.getSignedNumber(after.freeHoursPerDay - before.freeHoursPerDay)}h`,
        impact: this.getHealthImpact(before, after),
        explanation: this.getHealthExplanation(before, after),
  ...
  private getSalaryDimensionScore(before: OfferMetrics, after: OfferMetrics): number {
    ...
    return this.clamp((disposableDelta / 1000) * 12 + workOnlyDelta * 2 + realDelta * 2, -100, 100);
  }
  ...
  private getTimeDimensionScore(before: OfferMetrics, after: OfferMetrics): number {
    ...
    return this.clamp(freeTimeDelta + commuteDelta + monthlyDaysDelta, -100, 100);
  }
  ...
  private getHealthDimensionScore(before: OfferMetrics, after: OfferMetrics): number {
    const healthDelta = (after.healthFriendlyScore - before.healthFriendlyScore) * 22;
    const freeTimeDelta = (after.freeHoursPerDay - before.freeHoursPerDay) * 10;
    const commuteDelta = (before.commuteMinutesPerDay - after.commuteMinutesPerDay) / 4;
    return this.clamp(healthDelta + freeTimeDelta + commuteDelta, -100, 100);
  }
  ...
  private getEntertainmentExplanation(before: OfferMetrics, after: OfferMetrics): string {
    ...
    if (sceneDown && (budgetUp || freeTimeDown)) {
      return '跳槽后娱乐环境评分下降，同时娱乐预算压力或自由时间变差，娱乐体验大概率会下降。';
    }
  ...
  private getHealthImpact(before: OfferMetrics, after: OfferMetrics): 'positive' | 'negative' | 'neutral' {
    const score = this.getHealthDimensionScore(before, after);
    if (score > 8) return 'positive';
    if (score < -8) return 'negative';
    return 'neutral';
  }

  private getHealthExplanation(before: OfferMetrics, after: OfferMetrics): string {
    if (after.healthFriendlyScore > before.healthFriendlyScore && after.freeHoursPerDay >= before.freeHoursPerDay) {
      return '跳槽后工作强度对身心健康更友好，可持续性更强，不容易长期透支。';
    }
    if (after.workHoursPerDay > before.workHoursPerDay || after.workingDaysPerMonth > before.workingDaysPerMonth || after.commuteMinutesPerDay > before.commuteMinutesPerDay) {
      return '跳槽后工时、月工作天数或通勤更重，长期可能更影响恢复、情绪和身体状态。';
    }
    return '健康维度变化不算极端，但仍建议结合你对团队节奏和压力来源的主观感受判断。';
  }
  ...
  private generateNarrative(
    ...
    if (weightedScore >= 72) {
      return {
        ...
        summaryText: `最核心的原因在于「${lead?.label ?? '综合维度'}」明显改善，且「${second?.label ?? '关键维度'}」没有拖后腿。相比跳槽前，税后月收入 ${this.getSignedCurrency(after.monthlyIncome.税后工资 - before.monthlyIncome.税后工资)}，月可支配收入 ${this.getSignedCurrency(after.monthlyDisposable - before.monthlyDisposable)}。`,
      };
    }
    ...
    if (weightedScore >= 58) {
      return {
        ...
        summaryText: `这份新 offer 在「${lead?.label ?? '核心维度'}」上更适合你，但「${third?.label ?? '另一个维度'}」仍有明显权衡。它更像是“有条件成立”的选择：如果你愿意接受部分生活习惯调整，这次跳槽值得继续推进。`,
      };
    }
  ...
  private calculateHealthFriendlyScore(
    settings: Settings,
    params: {
      workHoursPerDay: number;
      commuteMinutesPerDay: number;
    ...
    const subjective = this.clamp(settings.healthFriendlyScore || 3, 1, 5);
    let objective = 5;

    if (params.workHoursPerDay >= 10.5) objective -= 1.2;
    else if (params.workHoursPerDay >= 9.5) objective -= 0.7;
    else if (params.workHoursPerDay >= 8.5) objective -= 0.3;

    if (params.workingDaysPerMonth >= 27) objective -= 1.1;
    else if (params.workingDaysPerMonth >= 25) objective -= 0.7;
    else if (params.workingDaysPerMonth >= 23) objective -= 0.3;

    if (params.commuteMinutesPerDay >= 120) objective -= 0.9;
    else if (params.commuteMinutesPerDay >= 90) objective -= 0.6;
    else if (params.commuteMinutesPerDay >= 60) objective -= 0.3;

    if (params.freeHoursPerDay < 4) objective -= 0.8;
    else if (params.freeHoursPerDay < 5) objective -= 0.4;

    return Number(this.clamp(subjective * 0.6 + objective * 0.4, 1, 5).toFixed(1));
  }
