...
const SmartWidgets: React.FC<SmartWidgetsProps> = ({
  ...
  const riskCount = comparison?.riskWarnings.filter((item) => item.level === '高').length ?? 0;
  const topFinding = comparison?.topPriorityFindings?.[0];
  const thresholds = comparison?.thresholdHints?.slice(0, 2) ?? [];

  return (
    <section className="grid grid-cols-1 gap-4 lg:grid-cols-12">
      <Card className="lg:col-span-5">
        <Eyebrow>Smart widget</Eyebrow>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">
          ...
          <MiniCard label="月可支配变化" value={comparison ? `${comparison.delta.monthlyDisposable > 0 ? '+' : ''}${comparison.delta.monthlyDisposable}` : '—'} />
          <MiniCard label="真实时薪变化" value={comparison ? `${comparison.delta.realHourlyPay > 0 ? '+' : ''}${comparison.delta.realHourlyPay}` : '—'} />
        </div>
      </Card>
      ...
      <Card className="lg:col-span-3">
        ...
        <div className="mt-5 space-y-2">
          {topPriorityKeys.map((key, index) => (
            <div key={key} className="flex items-center justify-between rounded-2xl bg-slate-50/90 px-4 py-3">
      ...
      <Card className="lg:col-span-4">
        ...
        <div className="mt-5 rounded-[24px] bg-[linear-gradient(135deg,#DCE8FF,#ECE7FB)] p-5">
      ...
      <Card className="lg:col-span-5">
        ...
        <div className="mt-5 space-y-3">
          {thresholds.length > 0 ? thresholds.map((item) => (
            <div key={item.label} className="rounded-2xl bg-slate-50/90 px-4 py-4">
            ...
            <div className="rounded-2xl bg-slate-50/90 px-4 py-4 text-sm leading-7 text-slate-500">
...
function Card({ className = '', children }: { className?: string; children: React.ReactNode }) {
  return (
    <div className={`rounded-[30px] border border-white/60 bg-white/72 p-5 backdrop-blur-xl shadow-[0_20px_60px_rgba(83,106,150,0.12)] md:p-6 ${className}`}>
      {children}
    </div>
  );
}
...
function ActionLink({
  ...
  return (
    <a
      href={href}
      className="group rounded-[24px] border border-slate-100 bg-slate-50/90 p-4 transition hover:-translate-y-0.5 hover:bg-white"
    >
      <div className="text-base font-semibold text-slate-950">{title}</div>
      <div className="mt-2 text-sm leading-7 text-slate-500">{desc}</div>
      <div className="mt-4 text-sm font-medium text-[#5B8CFF]">Open →</div>
    </a>
  );
}
