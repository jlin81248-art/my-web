    ...
    <section className="rounded-[28px] border border-white/60 bg-white/72 p-5 shadow-[0_18px_50px_rgba(83,106,150,0.12)] backdrop-blur-xl md:p-6">
      ...
      {breakdown.length === 0 ? (
        <div className="mt-5 rounded-[24px] bg-slate-50/90 px-5 py-5 text-sm leading-7 text-slate-500 dark:bg-slate-900/60 dark:text-slate-400">
          ...
          <code className="mx-1 rounded bg-slate-200 px-1 py-0.5 text-xs dark:bg-slate-800">breakdown</code>
          或
          <code className="mx-1 rounded bg-slate-200 px-1 py-0.5 text-xs dark:bg-slate-800">deltaBreakdown</code>
        ...
        <div className="mt-6 space-y-4">
          {breakdown.map((item) => {
            ...
            return (
              <div
                key={item.label}
                className="rounded-[22px] border border-slate-100 bg-slate-50/90 p-4 dark:border-slate-800 dark:bg-slate-900/60"
              >
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <div className="min-w-0">
                    <div className="text-base font-semibold text-slate-950 dark:text-slate-100">{item.label}</div>
                    <div className="mt-1 text-sm leading-7 text-slate-500 dark:text-slate-400">
