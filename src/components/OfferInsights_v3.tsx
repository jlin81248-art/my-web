...
const OfferInsights: React.FC<OfferInsightsProps> = ({ beforeLabel, afterLabel, comparison }) => {
  ...
  const cards = [
    ...
    {
      label: '偏好匹配分',
      value: `${comparison.weightedScore} / 100`,
      hint: comparison.summaryTitle,
      numeric: comparison.weightedScore - 50,
    },
  ];
        ...
        <div className="rounded-xl bg-gradient-to-r from-slate-900 to-blue-900 text-white p-4 md:p-5">
          ...
          <div className="mt-3 space-y-2">
            {comparison.topPriorityFindings.slice(0, 3).map((item, index) => (
              <div key={`${item.dimension}-${index}`} className="rounded-lg bg-white/10 px-3 py-2">
        ...
        <div className="rounded-xl border border-gray-100 dark:border-gray-700 bg-slate-50 dark:bg-slate-900 p-4 md:p-5">
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-sm">
            <span className="rounded-full bg-white dark:bg-gray-800 px-3 py-1 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200">
              住房负担率 {comparison.before.housingBurdenRate}% → {comparison.after.housingBurdenRate}%
            </span>
            <span className="rounded-full bg-white dark:bg-gray-800 px-3 py-1 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200">
              自由时间 {comparison.before.freeHoursPerDay}h → {comparison.after.freeHoursPerDay}h
            </span>
            <span className="rounded-full bg-white dark:bg-gray-800 px-3 py-1 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200">
              娱乐预算占比 {comparison.before.entertainmentBudgetRatio}% → {comparison.after.entertainmentBudgetRatio}%
            </span>
            <span className="rounded-full bg-white dark:bg-gray-800 px-3 py-1 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200">
              家庭负担率 {comparison.before.familyBurdenRate}% → {comparison.after.familyBurdenRate}%
            </span>
          </div>
        </div>
      </div>
