...
const subjectiveFieldMeta: Record<
  PreferenceKey,
  { label: string; key: keyof Settings; help: string }
> = {
  salary: {
    label: '薪酬满意度（1-5）',
    key: 'salarySatisfactionScore',
    help: '不是看绝对工资，而是你主观觉得这份薪酬是否值得这份投入。',
  },
  health: {
    label: '工作强度对身心健康友好度（1-5）',
    key: 'healthFriendlyScore',
    help: '越高代表越可持续，越不容易透支身体和情绪。',
  },
  entertainment: {
    label: '娱乐环境满意度（1-5）',
    key: 'entertainmentSceneScore',
    help: '综合看夜生活、线下活动、商圈和文娱供给是否符合你的偏好。',
  },
  housing: {
    label: '居住环境满意度（1-5）',
    key: 'housingEnvironmentScore',
    help: '综合看通勤便利、社区舒适度和你对住得怎么样的主观感受。',
  },
  time: {
    label: '时间掌控感（1-5）',
    key: 'timeFreedomScore',
    help: '你是否觉得自己的下班时间、节奏和可支配时间还算可控。',
  },
  family: {
    label: '家庭支持友好度（1-5）',
    key: 'familySupportScore',
    help: '综合看家庭支出压力、照顾安排和城市对家庭生活的友好程度。',
  },
  climate: {
    label: '气候舒适度（1-5）',
    key: 'climateComfortScore',
    help: '综合你对温度、湿度和长期宜居性的主观感受。',
  },
  growth: {
    label: '职业成长性（1-5）',
    key: 'growthScore',
    help: '综合行业机会、平台势能和你对长期成长的判断。',
  },
};
    ...
    <section className="rounded-2xl bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 p-4 md:p-6 space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          家庭支出直接输入总金额；主观体验评分只显示你当前最在意的 3 个维度，让分析更贴近你真正关心的问题。
        </p>
      </div>
      ...
      <div className="rounded-xl border border-blue-100 dark:border-blue-900/50 bg-blue-50/60 dark:bg-blue-950/20 p-4 space-y-4">
        <div>
          ...
          <div className="mt-1 text-xs text-blue-700/80 dark:text-blue-300/80">
            只展示你当前 TOP 3 优先级。这样结论不会固定围绕娱乐、气候、成长，而会跟着你的真实关注点变化。
          </div>
        </div>
