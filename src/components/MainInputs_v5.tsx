    ...
    <section className="rounded-2xl bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 p-4 md:p-6">
      <div className="mb-4">
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          年终奖会自动平均到每个月；系统同时展示“在岗时薪”和“含通勤真实时薪”。工作强度友好度会与工时、月工作天数、通勤一起影响健康维度判断。
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <div className="flex flex-col">
          <label htmlFor={`${title}-healthFriendlyScore`} className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">工作强度对身心健康友好度（1-5）</label>
          <select
            id={`${title}-healthFriendlyScore`}
            value={settings.healthFriendlyScore}
            onChange={(e) => onSettingChange('healthFriendlyScore', Number(e.target.value))}
            className="border border-gray-300 dark:border-gray-600 rounded-md py-2.5 px-3 text-sm bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200"
          >
            <option value={1}>1 - 高压透支</option>
            <option value={2}>2 - 偏高压</option>
            <option value={3}>3 - 中性</option>
            <option value={4}>4 - 比较可持续</option>
            <option value={5}>5 - 很友好</option>
          </select>
          <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">这是主观判断，系统会结合工时、月工作天数和通勤一起计算健康维度。</div>
        </div>
      </div>
    </section>
  );
