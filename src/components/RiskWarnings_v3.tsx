...
const levelStyles: Record<string, string> = {
  高: 'bg-red-50 text-red-700 border-red-200 dark:bg-red-900/20 dark:text-red-300 dark:border-red-800',
  中: 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-900/20 dark:text-amber-300 dark:border-amber-800',
  低: 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-300 dark:border-emerald-800',
};
    ...
    <section className="rounded-[28px] border border-white/60 bg-white/72 p-5 shadow-[0_18px_50px_rgba(83,106,150,0.12)] backdrop-blur-xl md:p-6">
      ...
      {safeWarnings.length === 0 ? (
        <div className="mt-5 rounded-[24px] bg-slate-50/90 px-5 py-5 text-sm leading-7 text-slate-500 dark:bg-slate-900/60 dark:text-slate-400">
          ...
          后续只要把 <code className="mx-1 rounded bg-slate-200 px-1 py-0.5 text-xs dark:bg-slate-800">riskWarnings</code>
        ...
        <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
          {safeWarnings.map((item, index) => (
            ...
            >
              <div className="flex items-center justify-between gap-3">
                ...
                <span
                  className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${
                    levelStyles[item.level] ?? 'bg-slate-100 text-slate-700 border-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700'
                  }`}
                >
                  {item.level}
                </span>
              ...
              <div className="mt-3 rounded-2xl bg-white/80 px-4 py-3 text-sm leading-7 text-slate-600 dark:bg-slate-950/70 dark:text-slate-300">
