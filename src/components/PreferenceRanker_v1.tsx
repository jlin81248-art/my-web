...
const weightHint = ['最高权重', '第二优先', '第三优先', '中等关注', '次要关注', '较低关注', '最低关注'];
  ...
  return (
    <section className="rounded-2xl bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 p-4 md:p-6 space-y-4">
      <div>
        ...
        <h2 className="mt-1 text-xl font-bold text-gray-900 dark:text-gray-100">你最看重什么</h2>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          把越在意的维度排得越靠前。后面的结论会优先围绕你的前 3 个痛点来分析。
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        {order.map((key, index) => (
          <div
            key={key}
            className="rounded-xl border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-4 flex items-start justify-between gap-4"
          >
            <div className="flex items-start gap-3">
              <div className="mt-0.5 h-8 w-8 rounded-full bg-blue-600 text-white text-sm font-bold flex items-center justify-center">
                {index + 1}
              </div>
              <div>
                <div className="text-base font-semibold text-gray-900 dark:text-gray-100">{labels[key].title}</div>
