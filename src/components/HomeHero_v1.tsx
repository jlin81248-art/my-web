...
const HomeHero: React.FC<HomeHeroProps> = ({
  ...
  const score = comparison?.weightedScore ?? 0;
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          <div className="rounded-[28px] border border-white/60 bg-white/72 p-5 backdrop-blur-xl shadow-[0_18px_50px_rgba(83,106,150,0.12)] lg:col-span-2">
            ...
            <div className="mt-6 grid grid-cols-3 gap-3">
              <MetricChip label="月可支配" value={comparison ? `${comparison.delta.monthlyDisposable > 0 ? '+' : ''}${comparison.delta.monthlyDisposable}` : '—'} />
              <MetricChip label="真实时薪" value={comparison ? `${comparison.delta.realHourlyPay > 0 ? '+' : ''}${comparison.delta.realHourlyPay}` : '—'} />
              <MetricChip label="主观体验" value={comparison ? `${comparison.topPrioritySubjectiveAfter.toFixed(1)}/5` : '—'} />
            </div>
          </div>
