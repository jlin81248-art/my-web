          ...
          <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                先填客观条件，再补充你当前最在意的 3 个维度的主观体验评分。后面的分析会优先围绕这 3 个点展开。
              </p>
            </div>
          </div>
        ...
        {comparison && beforeMetrics && afterMetrics && (
          <>
            <OfferInsights
              ...
              topPriorityKeys={topPriorityKeys}
            />
            <DeltaWaterfall breakdown={comparison.deltaBreakdown} />
            ...
            />
            <TimeHealthDrilldown
              beforeLabel={`${beforeOffer.sourceCity || '跳槽前'}`}
              afterLabel={`${afterOffer.sourceCity || '跳槽后'}`}
            ...
            <ScenarioSimulator
              ...
              comparison={simulatedComparison || comparison}
            />
            <ThresholdAnalysis hints={comparison.thresholdHints} />
            <RiskWarnings warnings={comparison.riskWarnings} />
            <FinalDecision comparison={simulatedComparison || comparison} />
          </>
        )}
      </main>
    </div>
